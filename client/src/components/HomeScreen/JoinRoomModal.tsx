...
type Props = {
    ...
    isVisible: boolean;
    closeModal: () => void;
    joinRoom: (room: JoinRoomInfo) => void;
}
...
function JoinPrivateRoomModal(props: Props) {
    ...
    function joinPrivateRoom() {
        if (props.isPrivate && roomPassword.current && roomPassword.current.value) {
            props.joinRoom({name: props.roomName, isPrivate: props.isPrivate, password: roomPassword.current.value});
            roomPassword.current.value = ""
        }
    }

    function joinPublicRoom() {
        props.joinRoom({name: props.roomName, isPrivate: false});
    }
        ...
        <ModalBase isVisible={props.isVisible} setIsVisible={props.closeModal}>
            <div className='relative p-4 pt-16 border-2 rounded-md border-gray-600 bg-zinc-900 bg-opacity-80 items-center text-center min-w-[300px] w-8/12 sm:w-3/12'>
                ...
                <button className='w-full p-2 bg-sky-600 rounded-md' 
                    onClick={props.isPrivate ? joinPrivateRoom : joinPublicRoom}>
                    Join
                </button>
